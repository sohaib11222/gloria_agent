...
export default function Support() {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: ({ ticketId, content, image }: { ticketId: string; content?: string; image?: File }) => {
      ...
      return supportApi.sendMessage(ticketId, { content, image })
    },
    onSuccess: () => {
  ...
  const handleSendMessage = () => {
    ...
    // Always send content - if no text but we have image, send empty string
    // If we have text, send the text. If only text, send text.
    const contentToSend = messageContent.trim() || (selectedImage ? '' : undefined)
    
    console.log('[Support] Sending message:', {
      hasContent: !!contentToSend,
      contentValue: contentToSend,
                  ...
                  {imagePreview && (
                    <div className="relative inline-block group">
                      <img 
                        src={imagePreview} 
                        alt="Preview" 
                        className="h-32 rounded-lg border-2 border-blue-300 shadow-sm object-cover" 
                      />
                      <button
                        onClick={() => {
                          setSelectedImage(null)
