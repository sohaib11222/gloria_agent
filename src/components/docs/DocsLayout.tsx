    ...
    <div className="docs-shell">
      ...
      <main className="docs-main">
        {showSdkGuide ? (
          ...
          <>
        {/* Getting Started block - only show when first endpoint is selected */}
        {selectedEndpoint && selectedEndpoint.id === categories[0]?.endpoints?.[0]?.id && (
          <section style={{ marginBottom: '2rem', padding: '1.5rem', background: 'linear-gradient(135deg, #dbeafe 0%, #e0e7ff 100%)', borderRadius: '0.5rem', border: '1px solid #bfdbfe' }}>
            <h2 style={{ marginBottom: '1rem', fontSize: '1.25rem', fontWeight: 600 }}>Getting Started</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div style={{ display: 'flex', alignItems: 'start', gap: '0.5rem' }}>
                <span style={{ fontSize: '1.25rem' }}>1️⃣</span>
                <span>Login to your agent account to get your API token</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'start', gap: '0.5rem' }}>
                <span style={{ fontSize: '1.25rem' }}>2️⃣</span>
                <span>Accept agreements from sources to enable availability</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'start', gap: '0.5rem' }}>
                <span style={{ fontSize: '1.25rem' }}>3️⃣</span>
                <span>Call Availability API to search for vehicles</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'start', gap: '0.5rem' }}>
                <span style={{ fontSize: '1.25rem' }}>4️⃣</span>
                <span>Create bookings using the offers you receive</span>
              </div>
            </div>
          </section>
        )}
